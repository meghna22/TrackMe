angular.module('trackme.services', [])
    .service('TrackSrv', function() {
        ...
        this.startRecord = function() {
            var self = this;
            var description = (new Date()).toString();

            var track = this.createTrack([], description);
            this.addTrack(track);

            intervalTimeoutRef = setInterval(function() {
                navigator.geolocation.getCurrentPosition(
                    function(pos) {
                        track.addPoint(pos.coords.latitude, pos.coords.longitude, (new Date()).getTime());
        ...
        this.stopRecord = function() {
            clearInterval(intervalTimeoutRef);
        };

        this.getTracksFromStorage = function() {
            var allTracks = localStorage.getItem("tracks");
            if(allTracks) {
                var trackObjects = JSON.parse(allTracks);

                this.initTracks();
                for (var i = 0; i < trackObjects.length; i++){
                    var t = this.createTrack([], trackObjects[i].description);
                    this.addTrack(t);
                    for (var j = 0; j < trackObjects[i].points.length; j++) {
                        var point = trackObjects[i].points[j];
                        t.addPoint(point.lat, point.long, point.time);
                    }
                }
            }
        };

        this.saveTracksToStorage = function() {
